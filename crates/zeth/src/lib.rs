...
// Ethereum-specific version that uses HeaderPot serialization
pub async fn run_eth<
    B: EthBlockBuilder<N, D, R, P> + Send + Sync + 'static,
    N: Network,
    D: Recoverable + 'static,
...
>(
    ...
    env_logger::init();
    info!("🔗 Starting run_eth - Using HeaderPot serialization for Ethereum");
    let cli = Cli::parse();
    let build_args = cli.build_args();
    // Fill default chain id if no way to infer it
    let chain_id = match (&build_args.rpc, build_args.chain) {
    ...
    // select a guest program - using Ethereum-specific method
    info!("📋 Building journal using Ethereum-specific method (HeaderPot compatible)");
    let expected_journal = B::build_journal_eth(
        chain_id,
        cache_dir.clone(),
        build_args.rpc.clone(),
        build_args.block_number,
    ...
    // preflight the block building process - using Ethereum-specific method with HeaderPot
    info!("🔄 Building blocks using HeaderPot serialization (no POT panics!)");
    let build_result = B::build_blocks_eth(
        chain_id,
        cache_dir.clone(),
        build_args.rpc.clone(),
        build_args.block_number,
    ...
    let computed_journal = if cli.should_prove() {
        ...
        let receipt = if let Ok(true) = Path::new(&file_name).try_exists() {
            ...
            info!("💡 Guest will use HeaderPot deserialization (data_from_parts_eth)");
            info!("Computing uncached receipt. This might take some time.");
            // run prover
            let network_name = String::from(network_name);
            let elf = elf.to_owned();
    ...
    info!("✅ run_eth completed successfully with HeaderPot serialization!");
    Ok(())
}
